...
| books M7 bank connectivity | ✔ | Statements IN from the formats banks worldwide emit - CAMT.053, MT940, OFX/QFX and CSV - detected from content, normalised to one signed-amount line shape, and deduped on the bank own reference (AcctSvcrRef, //bankref, FITID) where it gives one. Each format sign convention handled explicitly: CAMT CdtDbtInd inverted by a reversal flag, MT940 RC/RD where a reversed credit is money OUT, OFX TRNAMT trusted over TRNTYPE. Payments OUT as ISO 20022 pain.001, NACHA ACH (with its own entry hash and block arithmetic, refusing any record that is not 94 characters) or CSV, under a build/approve/export/settle discipline - approval names any payee the format cannot pay, and export and settle are separate because whether the bank accepted a file is not knowledge this system has. Nothing transmits: export hands the operator bytes. acc-bank-connection stores credentials through the estate AES-256-GCM vault, never returning them, and the live provider adapters (Open Banking, Plaid, TrueLayer, GoCardless) refuse clearly until implemented rather than returning an empty success that would read as an empty bank. Three new content types; suppliers gained bank details. smoke-books 102 checks green. |
| books M8 recurring revenue and collections | ✔ | The money that should come IN. Recurring-invoice templates issue real invoices through the same ar.createInvoice a typed one goes through - occurrence dates anchored on the START date and clamped to the month end, so a 31st billing day runs 31 Jan / 28 Feb / 31 Mar instead of walking backwards forever, and the occurrence number is the idempotency key so a sweep that fires twice or catches up after an outage bills nobody twice (catch-up capped at 24). The collections ladder walks every open invoice past due - judged on due date and balance, never on invoice status - and writes one notice per invoice per rung, ever; a queued notice whose invoice has since been paid is withdrawn before anyone can send it. Rungs are configurable and configuring none leaves a built-in 7/14/30/60/90 ladder in force. Nothing sends: a notice is written Queued and a person marks it sent. Credit control finally acts on the limits the estate could already compute - POS pay-later consults the gate BEFORE moveStockItems, because after it a refusal would be a lie the inventory has acted on; a refusal blocks the sale, a broken check does not, and an override needs a reason that rides onto the invoice. A blank credit limit means no limit, not zero headroom; a live Hold Credit notice outranks the arithmetic. Also closed: nothing ever set an invoice to Overdue. Four new content types, migration 033, two nightly crons. smoke-books 137 checks green; books app 19 routes; verify-wiring 30/30, verify-docs and verify-encoding clean. |
| books M9 fixed assets | ✔ | The register for things OWNED rather than sold. Registering an asset posts NOTHING - the purchase already hit the books through its bill, and posting the cost again would put it on the balance sheet twice; the one exception, an asset acquired outside the system, says so explicitly with a funding account. Straight line is computed from a cumulative target rather than by repeating a monthly figure, so 1000 over 3 months charges 333.33/333.34/333.33 and lands exactly instead of leaving a penny or a fat final month. Reducing balance terminates: it approaches salvage asymptotically, so the schedule stops at the useful life and the last period takes what is left. Full-month convention, stated rather than implied. Disposal catches the charge up to the disposal date FIRST, because months of unrecorded depreciation would otherwise land in the gain and dress a wasting asset up as a profit on sale; a write-off is the same verb with nothing coming back. Five seeded accounts including 1590 Accumulated Depreciation as a contra ASSET (credit normal balance, the shape 4100 Sales Returns already had) so the balance sheet nets it down with no special casing. Asset numbers are the estate first permanent series - no yearly reset, no branch scope - because the number goes on a sticker. Three new content types, migration 034, a monthly cron. Also fixed: postOrCapture returns valid:false rather than throwing, and the first version of the charge ignored it, recording a month as charged with no journal behind it; every posting site here now checks the verdict. smoke-books 162 checks green. |
| books M10 budgets and variance | ✔ | What the year was SUPPOSED to look like. A budget is stored one figure per account per MONTH, never annually: an annual figure with a twelfth taken on demand cannot express a seasonal business, and a seasonal business is exactly the one that needs a budget. Typing an annual number still works - it is spread, with optional weights - and the spread uses a cumulative target so the months add to exactly the year rather than to 999.96. Variance is signed by the account own direction and flagged favourable by its type, because revenue 10 percent under budget and spend 10 percent under budget are opposite news and one colour must not mean both; balance-sheet accounts get null rather than a guess. Actuals come from reports._lines, the same posted journal lines every other report reads. A budgeted month overlapping the window counts in full - the month is the unit the figure was agreed in. Copy-forward with an uplift covers the same NUMBER of months, not the same calendar dates. Locking refuses edits, because the number performance is measured against must not move after the fact. Surface in ACCOUNTS, not books - budgets are the accountant workbench. Two new content types. smoke-books 180 checks green. |
| books M11 dimensions | ✔ | Cost centre and project on the journal line - OPTIONAL everywhere, forever, because 27 posting sites existed before this and none supplied one; an undimensioned line is the normal case, and the posting contract suite passing untouched is the proof. Project profitability is READ from the ledger every time rather than accumulated on the project row: a cached total is one more number to drift, and this program has already had to build a nightly job to repair one. The project row holds what was AGREED - contract value, budget cost - and everything actual is derived; where nothing was agreed the report answers null, because no contract value recorded and billed exactly what we said are different answers. The cost-centre report gives undimensioned lines their own Unallocated row, sorted last, because hiding them would make an adoption look complete when it is not. Also closes the payroll cost-centre split: the accrual was one lump on PAYROLL_EXPENSE and is now one debit per centre, with any rounding residual landing on the largest line rather than the smallest. Two new content types. smoke-books 191 checks. |
| books M12 foreign exchange | ✔ | Rates as base units per ONE foreign unit, looked up as the latest published ON OR BEFORE a date - rates arrive irregularly and an exact-date lookup fails on precisely the days a month-end falls on. Same currency and date twice is an update, not a second row, because two rates for one day makes revaluation depend on insertion order. Revaluation posts the DELTA since each item was last revalued, never a revaluation-and-reversal pair: the textbook method doubles the entries and a reversal that fails to run leaves the books wrong for a month, while the delta is idempotent by construction - running it twice on one date posts nothing, and a rate falling back posts only the fall. Gain and loss are SEPARATE accounts (4500/6900) because a year that made 50k and lost 48k is a different story from one that netted 2k. Without RUTBA_BASE_CURRENCY configured it refuses rather than guessing. Exposure answers null where a rate is missing. Two new content types. smoke-books 207 checks. |
| books M13 three-way match | ✔ | Ordered against received against billed. Quantity and value variances are reported SEPARATELY, because a bill 200 over could be ten extra units at the agreed price or the agreed units at an inflated one. Tolerances are two-sided - a percentage AND an absolute floor, both of which must be breached - since a percentage alone flags every rounding penny on a small order and an absolute alone waves through 2 percent of a large one. An open exception BLOCKS the payment run, and only a person accepting it with a reason clears it; a control anything automatic could clear is not a control. Bills with no purchase order were never part of a match and are never blocked by one. Not-yet-received and not-yet-billed are STATES, not exceptions. Matching compares the bill subtotal, because a PO is agreed before tax. One new content type. smoke-books 219 checks. |
| books M14 quotes | ✔ | The last document in the sales chain: a price offered, good until a date. A quote posts NOTHING - an offer is not a transaction, and a quote that touched the ledger would book revenue for every job the business did not win; the only posting is the invoice accept creates, through ar.createInvoice like every other invoice, so a quoted price and a billed price cannot drift. Expiry is DERIVED on read rather than swept by a nightly job, because a quote accepted at 9am on the day it expired must not depend on when a job ran; accepting a lapsed quote is refused rather than quietly honoured. accept is idempotent - clicking twice must never mean billed twice. Two new content types, and a permanent QT series. smoke-books 235 checks green. THE BOOKS PROGRAM IS COMPLETE: every deferred item is built. |
...
- **Comms interaction emission:** meet/calls modules emit core events and carry
  ...
  meetings and calls reach the consumer record timeline.
