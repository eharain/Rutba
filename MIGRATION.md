...
**What belongs here, and what does not.** One thing moved, or one thing was ported
onto core, per row — with what was verified about it at the time. That is all.
**Product milestones do not belong here**, however large: they belong to their
programme's own file, where the owner decisions and the full sequence live.
The distinction had already failed once — nine books milestones from 2026-08-31,
some 12KB of them, had accumulated in a ledger whose subject ended on 08-25 and
which says in its first line that it is frozen. They duplicated
[the books programme](consumer/docs/todo/books-program/README.md) at lower
fidelity, and were collapsed back to a single row on 2026-09-01.

If you are about to add a row because work landed, ask what it is a record *of*.
A move belongs here. A feature belongs where its programme is tracked, and
[PLAN.md](PLAN.md) §8 is where the estate's running “landed since” lives.
...
| books → finance module + app | ✔ | The finance group’s first `api/` module: `consumer/finance/api/books` mounted on core, and a new `books` app (unit `rutba_books`, `finance/apps/books`, :4024) on `erp.gl`+`erp.ap-ar`, so the portal’s existing books listing lights both finance apps. That is the migration-relevant fact. **The program itself — M0 through M15, and what each one built — is [consumer/docs/todo/books-program/README.md](consumer/docs/todo/books-program/README.md)**, which carries all 23 milestones with their owner decisions. Nine of them were duplicated here at lower fidelity until 2026-09-01; product milestones do not belong in a migration ledger, and this row is the boundary. |
