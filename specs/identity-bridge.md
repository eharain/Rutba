...
| 5 | **Migration 114.** `114-up-users-rutba-sub-ensure` with `requires: ['up_users']`, guarded and one-way: a database migrated before Strapi made the table defers it rather than recording it, and gets the column when the table arrives. 112 untouched. Ordinals claimed in both READMEs. Read again on 2026-09-22 against the migrations directory itself: **last taken 114, next free 115, nobody holding one** - the 115 WS-C had claimed for C11 was released unminted when that door reused `strapi_core_store_settings` (consumer `73b200ed`, records `6850c37`). | consumer 1c823a04 |
