...
| 7. Ordinal record | `api/core/migrations/README.md` carries the next ordinal at its top and is read again at merge time - which it needed three times this round, as 115 became WS-C's for C11 and then came free again when that door reused `strapi_core_store_settings` and minted nothing. It now says **next free 115**, last taken 114 (WS-D), checked against the migrations directory rather than against any document; `specs/README.md` agrees in both the places it states one (records `6850c37`). Commits `8b1bb9db`, `6a01af56`, `6d15270e`, `d2eef64d`, `73b200ed`. |
| 8. Disclosure | Below. |
